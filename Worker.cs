            ...
            try
            {
                using (GCCEntities gcc = new GCCEntities())
                {
                    ...
                    foreach (var item in pnLInputModels)
                    {
                       ...
                       // var query = $"EXEC SpTax_GeneratePandL_V3 {item.RefId},{item.ClientId},'{item.FiscYear}','','N','N'";
        ...
        private string GeneratePnLExcel(string fileName, CLIENT client, string start, string end, ClosingRateForLongTerCapGainasJanuary closingRateForLongTerCapGainasJanuary, PnLInputModel input)
        {
            using (var wbook = new XLWorkbook())
            {
                ...
                try
                {
                    ...
                    //        var dataheadings = "Symbol,Description,ISIN,Entry Date (Buy),Exit Date (Sell),Quantity,Buy Value,Sell Value,Buy & Sell Expense ,Net Profit/Loss ,Period of Holding (Days)";
                    //        var BondSummayHeadings = "Segment,Type,Buy Value,Sell Value,Expense,Net Profit/ Loss";
                    //        var taxHeadings = "Buy Brokerage,Buy GST,Buy Exchange Levy,Buy Stamp Duty,Total Buy Cost,Sale Bokerage,Sale GST,Sale Exchange Levy,Sale Stamp Duty,Total Sell Cost";
                    //        string[] bondSummayHeadingsArray = BondSummayHeadings.Split(',');

                    //        var equityResult =


                    //       var equityResult = gcc.Database.SqlQuery<Tax_Profit_Details_Cash>($"SpTaxPandLExcel_V2 {input.RefId},{input.ClientId},'{input.FiscYear}','Bond'").ToList();
                    //        var groupedData = equityResult.GroupBy(p => p.Type);
                    ...
                    //            ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, i + 2, groupedItem.Key.ToLower() == "speculation" ? "Equity (Intraday/Speculative)" : "Equity(" + groupedItem.Key + ")");
                    //            ExcelUtils.FormatCell(xlBondWorkSheet, lastRow, i + 2, false);
                    ...
                    //                    for (i = 0, j = 18; i < dataheadingsArray.Length - 2; i++)
                    //                    {
                    //                        ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, i + 2, dataheadingsArray[i]);
                    //                        ExcelUtils.FormatCell(xlBondWorkSheet, lastRow, i + 2, true);
                    //                    }
                    //                }
                    //                else
                    ...
                    //            for (i = 0, j = 18; i < taxHeadingsArray.Length; i++)
                    //            {
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, j++, taxHeadingsArray[i]);
                    ...
                    //                        && item.TranDateSale.GetValueOrDefault().Date >= new DateTime(2018, 2, 1).Date)
                    //                    {
                    //                        ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, closingRateForLongTerCapGainasJanuaryList.Any(p => p.Security.Trim().ToLower() == item.Security.Trim().ToLower()) ?
                    //                        closingRateForLongTerCapGainasJanuaryList.FirstOrDefault(p => p.Security.Trim().ToLower() == item.Security.Trim().ToLower()).Rate.ToString() : null);
                    //                        ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, closingRateForLongTerCapGainasJanuaryList.Any(p => p.Security.Trim().ToLower() == item.Security.Trim().ToLower()) ?
                    //                            closingRateForLongTerCapGainasJanuaryList.FirstOrDefault(p => p.Security.Trim().ToLower() == item.Security.Trim().ToLower()).Rate * item.SaleQty : null);
                    ...
                    //                    else
                    //                    {
                    //                        ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, "NA");
                    //                        ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, "NA");
                    //                    }
                    //                }
                    ...
                    //                        if (tempAmount == item.SaleValue.Value)
                    //                        {
                    //                            ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, 0);
                    //                        }
                    //                        else
                    //                        {
                    //                            ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, item.SaleValue - (tempAmount + item.PurchaseBrokerage + item.PurchaseExchangeLevy
                    //                                                          + item.PurchaseServiceTax + item.PurchaseStampDuty
                    //                                                          + item.SaleBrokerage + item.SaleExchangeLevy +
                    //                                                          item.SaleServiceTax + item.SaleStampDuty));
                    ...
                    //                if (groupedItem.Key.ToLower() == "speculation")
                    //                    ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, Math.Abs(item.SaleValue.GetValueOrDefault() - item.BuyValue.GetValueOrDefault()));
                    //                else if (groupedItem.Key.ToLower() == "long term" || groupedItem.Key.ToLower() == "short term")
                    //                    ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, ++i, Math.Abs(item.SaleValue.GetValueOrDefault()));
                    ...
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, i++, taxableProfit);
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, taxableProfitSummaryRow, taxableProfitSummarycolumn, taxableProfit);
                    //                ExcelUtils.SetValueToCell(xlSummaryWorkSheet, summaryLastRow, summaryColumnIndex++, taxableProfit);
                    //            }
                    //            ExcelUtils.FormatCell(xlBondWorkSheet, lastRow, i - 1, true, "grey");
                    ...
                    //            if (groupedItem.Key.ToLower() == "speculation")
                    //            {
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, i++, groupedItem.Sum(p => Math.Abs(p.SaleValue.GetValueOrDefault() - p.BuyValue.GetValueOrDefault())));
                    //                ExcelUtils.FormatCell(xlBondWorkSheet, lastRow, i - 1, true, "grey");
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, turnOverSpeculationSummaryRow, turnOverSpeculationSummaryColumn + 1, groupedItem.Sum(p => Math.Abs(p.SaleValue.GetValueOrDefault() - p.BuyValue.GetValueOrDefault())));
                    //            }
                    //            else if (groupedItem.Key.ToLower() == "long term")
                    //            {
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, i++, Math.Abs(groupedItem.Sum(p => p.SaleValue.GetValueOrDefault())));
                    //                ExcelUtils.FormatCell(xlBondWorkSheet, lastRow, i - 1, true, "grey");
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, taxableProfitSummaryRow, taxableProfitSummarycolumn + 1, Math.Abs(groupedItem.Sum(p => p.SaleValue.GetValueOrDefault())));
                    //            }
                    //            else if (groupedItem.Key.ToLower() == "short term")
                    //            {
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, lastRow, i++, Math.Abs(groupedItem.Sum(p => p.SaleValue.GetValueOrDefault())));
                    //                ExcelUtils.FormatCell(xlBondWorkSheet, lastRow, i - 1, true, "grey");
                    //                ExcelUtils.SetValueToCell(xlBondWorkSheet, turnOverShortTermSummaryRow, turnOverShortTermSummaryColumn + 1, Math.Abs(groupedItem.Sum(p => p.SaleValue.GetValueOrDefault())));
                    //            }
                    ...
                    #region Opening Pos
                    var xlOpeningPosWorkSheet = wbook.Worksheets.Add("Opening Pos");
                    {
                        using (GCCEntities gcc = new GCCEntities())
                        {
                            gcc.Database.CommandTimeout = queryTimeout;
                            ...
                            var equityResult =  gcc.Database.SqlQuery<Pos>($"SpTax_GeneratePandL_OpeningPos {input.ClientId},'{input.FiscYear}'").ToList();
                            var groupedData = equityResult.GroupBy(p => p.Type);
                            ...
                            lastRow = 11;
                            ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, i + 1, "Open Position as on " + Convert.ToDateTime(end).ToString("dd-MM-yyyy") +"(PY Closing Position)");
                            ...
                            foreach (var groupedItem in groupedData)
                            {
                                var foFutures = groupedData.SelectMany(group => group).Where(p => p.Instrument.StartsWith("FUT"));
                                var foOptions = groupedData.SelectMany(group => group).Where(p => p.Instrument.StartsWith("OPT"));
                                string[] OptionheadingsArray = Optionheadings.Split(',');
                                string[] FutureheadingsArray = Futureheadings.Split(',');
                                subheading = (groupedItem.Key.ToLower() == "eq") ? "Equity" : "Currency";
                                ...
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, ++i, true, "grey");
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, ++i, true, "grey");
                                ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, foOptions.Sum(p => p.SQPurchaseValue));
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, i, true, "grey");
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, i, true, "grey");
                                ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, foOptions.Sum(p => p.SQSaleValue - p.SQPurchaseValue));
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, i, true, "grey");
                                ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, foOptions.Sum(p => Math.Abs(p.SQSaleValue - p.SQPurchaseValue)));
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, i, true, "grey");
                                ...
                                foreach (var futures in foFutures)
                                {
                                    ...
                                    ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, futures.ActualSalesQty);
                                    ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, futures.SQPurchaseValue);
                                    ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, futures.SQSaleValue);
                                    ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, futures.SQSaleValue - futures.SQPurchaseValue);
                                    ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, Math.Abs(futures.SQSaleValue - futures.SQPurchaseValue));
                                    i = 1;
                                    lastRow++;
                                }
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, ++i, true, "grey");
                                ...
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, ++i, true, "grey");
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, ++i, true, "grey");
                                ExcelUtils.SetValueToCell(xlOpeningPosWorkSheet, lastRow, ++i, foFutures.Sum(p => p.SQPurchaseValue));
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, i, true, "grey");
                                ExcelUtils.FormatCell(xlOpeningPosWorkSheet, lastRow, i, true, "grey");
