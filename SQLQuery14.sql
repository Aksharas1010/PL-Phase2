alter Procedure [dbo].[SpTax_GeneratePandL_phase2]  
(                           
 @RefId int,  
 @cid Int,  
 @FiscYear varchar(12),  
...
AllocatedPurchaseQty Numeric(15, 2) Not Null  default 0 ,                        
 ...
 (                                          
  ...
  Create index idxSec on #TempBB(Security) 
...
(#Transactionno.buysell='B')                                
 ...
 from    #psp_sauda (Nolock)                                    
 Where (Buysell = 'B') and (Saudatype = 'NOR')  and GROUPCODE != 'BB'       
  and (delserviceqty>0)   
 ...
 from #psp_sauda (Nolock)                                    
 Where (Buysell = 'S') and (Saudatype = 'NOR')  and  GROUPCODE != 'BB'       
  and (delserviceqty>0)                           
 Group By Security,Trandate                   
...
--End  
 ...
 Insert  into #TempBB (Security,Trandate,SaudaType,BuyQty,BuyValue,Brokerage,ServiceTax,ExchangeLevy,StampDuty,Buysell)  /*-*/                                        
 Select Security , Trandate , 'NORMAL TRADE SPEC' ,                                          
  Sum(Qty-delserviceqty) , Sum((Qty-delserviceqty) * Rate),                         
  Sum((Qty-delserviceqty) * Brokerage), Sum((Qty-delserviceqty) * (Servicetax/qty) ) ,                    
  --+ Sum((Servicetax/qty)*(Qty-delserviceqty) ) * @Ecess ,                                                    
 ...
 Where (Buysell = 'B')  and (Saudatype = 'NOR' )    and  GroupCode = 'BB'            
 Group By Trandate,Security ,Buysell                           
 Insert  into #TempBB (Security,Trandate,SaudaType,SellQty,SellValue,Brokerage,ServiceTax,ExchangeLevy,StampDuty,Buysell) /*-*/                                              
 Select  Security ,Trandate ,'NORMAL TRADE SPEC' ,                                          
  Sum(Qty-delserviceqty) ,Sum((Qty-delserviceqty) * Rate) ,                                     
  Sum((Qty-delserviceqty) * Brokerage),Sum((Servicetax/qty)*(Qty-delserviceqty) )  ,                    
  -- + Sum((Servicetax/qty)*(Qty-delserviceqty) ) * @Ecess,                                          
 ...
 Where (Buysell = 'S')  and (Saudatype = 'NOR' ) and  GroupCode = 'BB'             
 Group By Trandate,Security ,Buysell                                                                        
 Delete from #TempBB where Trandate<@finstart                   
 Delete from #TempBB where Trandate>@todate       
 Select Trandate,Security,Buysell,Sum(BuyQty) BuyQty, Sum(BuyValue) BuyValue,Sum(SellQty) SellQty,                                     
  Sum(SellValue) SellValue, Sum(SellValue)-Sum(BuyValue) Profit,'N' Exist ,                        
  Sum(Brokerage) Brokerage,Sum(ServiceTax) ServiceTax ,Sum(ExchangeLevy) ExchangeLevy,              
  Sum(StampDuty) StampDuty,Sum(StampDuty) derivedsduty                    
 ...
 from  #TempBB                                   
 Group by Trandate,Security,Buysell                                                   
 Alter table #TempProfitBB add Expense Numeric(15,4)                                                
 Update #TempProfitBB set profit  = profit- (Brokerage+ServiceTax+ExchangeLevy+StampDuty),                        
 Expense =  (Brokerage+ServiceTax+ExchangeLevy+StampDuty)  
 ...
 Into #TempProfitBuyBack                        
 from #TempProfitBB A,#TempProfitBB B                        
...
Insert into #profit select 'BuyBack',ISNULL(sum(profit),0) from #TempProfitBuyBack
...
Alter table #TempProfitBuyBack add ISIN varchar(32)  , Description varchar(126)     -- Added by Abdul Samad On 19.07.2017 for GBNPP_SUP-1542                                       
           
 update #TempProfitBuyBack set ISIN=s.isinno,Description = S.Description         
 from #TempProfitBuyBack t,security s (nolock)               -- Added by Abdul Samad On 19.07.2017 for GBNPP_SUP-1542          
 where t.security=s.security 
 ...
 Insert into Tax_Profit_Details_Cash                        
  ...
  PurchaseExchangeLevy,PurchaseStampDuty,BuyExpense,SaleBrokerage,SaleServiceTax,        
  SaleExchangeLevy,SaleStampDuty,SellExpense,Description)                        
 select @RefId,@cid,'BuyBack',Trandate,Security,ISIN,CEILING(BuyQty),BuyValue,Trandate,        
  CEILING(SellQty),SellValue,Profit,'System',getdate(),        -- Added by Abdul Samad On 05.05.2017 CEILING added for Buy and sell qty          
  BuyBrokerage,BuyServiceTax,BuyExchangeLevy,BuyStampDuty,BuyExpense,SaleBrokerage,        
  SaleServiceTax,SaleExchangeLevy,SaleStampDuty,saleExpense ,Description                 
 from #TempProfitBuyBack order by security  
